  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-medium text-white  tracking-tighter uppercase">Stat Leaderboard</h1>
          <p className="text-sm text-gray-400">Comprehensive historical performance tracking from 1985-2025.</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-1 bg-white/5 p-1 rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] backdrop-blur-xl">
             <button 
               onClick={() => setSeasonType("Regular Season")}
               className={`px-6 py-2.5 rounded-xl text-xs font-medium uppercase tracking-widest transition-all ${seasonType === "Regular Season" ? 'bg-orange-500 text-white shadow-none' : 'text-gray-500 hover:text-gray-300'}`}
             >
               Regular
             </button>
             <button 
               onClick={() => setSeasonType("Playoffs")}
               className={`px-6 py-2.5 rounded-xl text-xs font-medium uppercase tracking-widest transition-all ${seasonType === "Playoffs" ? 'bg-blue-600 text-white shadow-none' : 'text-gray-500 hover:text-gray-300'}`}
             >
               Playoffs
             </button>
          </div>

          <div className="flex items-center gap-3 bg-white/5 border border-zinc-200 dark:border-zinc-800 border-[0.5px] p-1.5 rounded-2xl backdrop-blur-xl relative">
             <Calendar className="ml-3 text-orange-500" size={16} />
             <select 
               value={season}
               onChange={(e) => setSeason(e.target.value)}
               className="bg-transparent text-sm font-medium text-white px-3 py-2 outline-none cursor-pointer appearance-none min-w-[120px] relative z-10"
             >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <details className="lg:hidden mb-4 bg-zinc-100 dark:bg-zinc-800   border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl overflow-hidden">
            <summary className="p-4 text-sm font-medium text-white uppercase tracking-widest cursor-pointer flex items-center justify-between">
              ...
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-medium transition-all flex items-center justify-between group ${
                    category === cat
                    ? 'bg-orange-500 text-white shadow-none shadow-none'
          ...
          <div className="hidden lg:block bg-zinc-100 dark:bg-zinc-800   border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl p-4 shadow-none sticky top-24">
        ...
        <div className="lg:col-span-3 space-y-6">
               ...
               {[1, 0, 2].map(idx => {
                 ...
                 return (
                   <div 
                    ...
                    className={`relative bg-zinc-100 dark:bg-zinc-800 ${color} to-transparent border rounded-3xl p-6 text-center shadow-none hover:shadow-none hover:shadow-none overflow-hidden group hover:scale-105 transition-transform duration-200 cursor-pointer`}
                   >
                      <div className="absolute top-0 right-0 p-4">
                         <div className={`text-4xl font-medium opacity-10  tracking-tighter ${idx === 0 ? 'text-yellow-500' : 'text-white'}`}>{rank}</div>
                      </div>
                      ...
                      <h3 className="text-xl font-medium text-white  tracking-tight">{p.PLAYER_NAME}</h3>
                      ...
                      <div className="inline-block bg-white/5 backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl px-6 py-2">
                         <span className="text-2xl font-medium text-white tracking-tighter ">{getStatValue(p)}</span>
          ...
          {/* List View */}
          <div className="bg-zinc-100 dark:bg-zinc-800  to-black border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-3xl overflow-hidden shadow-none relative">
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px] flex flex-col md:flex-row md:items-center justify-between gap-4">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-medium text-white  uppercase tracking-tight">Stat Leaderboard</h2>
               ...
               <div className="relative group">
                 ...
                 <input 
                   ...
                   className="w-full rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white/5 py-2.5 pl-12 pr-4 text-sm font-medium text-white outline-none transition-all focus:border-orange-500/50 focus:ring-4 focus:ring-orange-500/10 sm:min-w-[240px]"
                 />
               </div>
            </div>
            ...
            <div className="w-full overflow-x-auto rounded-xl">
              {loading ? (
                ...
                <table className="w-full min-w-[600px] text-left text-xs sm:text-sm">
                  <thead>
                    <tr className="bg-white/[0.02] text-[10px] font-medium text-gray-500 uppercase tracking-[0.2em] border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                      <th className="py-1.5 px-2 text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Rank</th>
                      <th className="py-1.5 px-2 text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Player</th>
                      <th className="py-1.5 px-2 hidden sm:table-cell text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Team</th>
                      <th className="py-1.5 px-2 hidden sm:table-cell text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Games</th>
                      <th className="py-1.5 px-2 text-right text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Value</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {filteredLeaders.map((p, idx) => (
                      ...
                      >
                        <td className="py-2 px-2 text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                          {idx < 3 ? (
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-medium  shadow-none ${
                              idx === 0 ? 'bg-zinc-100 dark:bg-zinc-800   text-black' :
                              idx === 1 ? 'bg-zinc-100 dark:bg-zinc-800   text-black' :
                              'bg-zinc-100 dark:bg-zinc-800   text-white'
                            }`}>
                               {idx + 1}
                            </div>
                          ) : (
                            <span className="text-sm font-medium  tracking-tighter text-gray-500 group-hover:text-gray-300 transition-colors">
                              #{idx + 1}
                            </span>
                          )}
                        </td>
                        <td className="py-2 px-2 text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                          <div className="flex items-center gap-3">
                            <div className="relative shrink-0">
                              ...
                              <img
                                src={getPlayerHeadshotUrl(p.PLAYER_ID!)}
                                className="w-10 h-10 rounded-full border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-gray-800 relative z-10"
                                onError={(e) => e.currentTarget.src = 'https://www.nba.com/assets/logos/teams/primary/web/NBA.svg'}
                              />
                            </div>
                            <div>
                               <p className="text-sm font-medium text-white  group-hover:text-orange-400 transition-colors">{p.PLAYER_NAME}</p>
                               <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest sm:hidden">{season === 'Lifetime' ? 'Career' : p.TEAM_ABBREVIATION}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-2 px-2 hidden sm:table-cell text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                          {season !== 'Lifetime' && p.TEAM_ID !== 0 ? (
                            <div className="flex items-center gap-2">
                              <img src={getTeamLogoUrl(p.TEAM_ID)} className="w-6 h-6 object-contain" />
                              <span className="text-xs font-medium text-gray-400">{p.TEAM_ABBREVIATION}</span>
                            </div>
                          ) : (
                            <span className="text-xs font-medium text-gray-500  uppercase">NBA Legend</span>
                          )}
                        </td>
                        <td className="py-2 px-2 hidden sm:table-cell text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                ...
                <div className="h-[300px] flex flex-col items-center justify-center text-center p-12">
                   <div className="bg-white/5 p-4 rounded-full mb-4 border border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                      <Search className="text-gray-600" size={32} />
                   </div>
                   <h3 className="text-xl font-medium text-white  uppercase mb-2">No Leaders Found</h3>
      ...
      {selectedPlayer && (
        <div id="leaderboard-player-modal" className="fixed inset-0 z-[100] overflow-y-auto bg-white dark:bg-zinc-900 backdrop- p-4 animate-in fade-in duration-300">
           <div className="flex min-h-full items-end sm:items-center justify-center">
              ...
              <button 
                ...
                className="absolute top-6 right-6 text-gray-500 hover:text-white bg-white/5 p-2.5 rounded-full z-20 transition-all border border-zinc-200 dark:border-zinc-800 border-[0.5px] hover:bg-white/10"
              >
                 ...
                 <div className="absolute -bottom-12 left-10 flex items-end gap-6 z-10">
                    <div className="relative">
                       ...
                       <img 
                         ...
                         className="w-32 h-32 object-cover rounded-full border-4 border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-gray-800 shadow-none relative z-10"
                       />
                       {season !== 'Lifetime' && (
                    ...
                    <div className="mb-14">
                       <h2 className="text-4xl font-medium text-white  uppercase tracking-tighter">{selectedPlayer.PLAYER_NAME}</h2>
              ...
              <div className="p-10 pt-16">
                 ...
                 <div className="mt-10 flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest">Advanced Rating</p>
                          <p className="text-2xl font-medium text-white ">{(selectedPlayer as any).PER?.toFixed(2) || 'N/A'}</p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className="text-[10px] font-medium text-gray-500 uppercase tracking-widest">True Shooting</p>
                       <p className="text-2xl font-medium text-emerald-400 ">{((selectedPlayer as any).TS_PCT * 100).toFixed(1)}%</p>
                    </div>
                 </div>
              </div>
           </div>
...
function StatBox({ label, value, sub, color }: any) {
  return (
    <div className="bg-white/5 border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-3xl p-6 text-center group hover:bg-white/[0.08] transition-all">
       ...
       <div className="flex items-baseline justify-center gap-1">
          ...
          <span className="text-[10px] font-medium text-gray-500 uppercase">{sub}</span>
       </div>
    </div>
  );
}
...
function MiniStat({ label, value }: any) {
  return (
    <div className="bg-white/[0.02] border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl p-4 text-center">
