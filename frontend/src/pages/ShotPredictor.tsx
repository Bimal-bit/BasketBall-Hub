  ...
  return (
    <div className="w-full space-y-6">
      <div className="p-4 border-b border-gray-800 mb-4">
        <h1 className="text-base font-medium text-gray-500 text-white">Shot tools</h1>
        <p className="text-xs text-gray-500">Official NBA API player logs and shot charts, with probability estimation</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 shadow-none">
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full bg-gray-800 border border-gray-800 rounded-lg py-2 pl-9 pr-3 text-sm text-white outline-none focus:border-orange-500"
              />
            </div>
            <select
              value={selectedPlayer?.id ?? ''}
              onChange={event => setSelectedPlayer(players.find(player => player.id === Number(event.target.value)) ?? null)}
              className="w-full bg-gray-800 border border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-orange-500"
            >
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4 shadow-none">
            <div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 text-center shadow-none hover:scale-[1.02] transition-all duration-200 hover:shadow-none">
            {loadingPlayer ? (
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 shadow-none">
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-2 gap-2 mb-3">
                {SHOT_ZONES.map(zone => (
                  <button key={zone} onClick={() => setSelectedZone(zone)} className={`rounded-lg border px-2 py-1.5 text-[10px] text-left transition-all duration-200 hover:scale-[1.02] ${selectedZone === zone ? 'border-orange-500/50 bg-orange-500/10 text-orange-200' : 'border-gray-800 bg-gray-800/30 text-gray-400 hover:border-gray-800'}`}>
                    {zone}
                  </button>
                ))}
              </div>
