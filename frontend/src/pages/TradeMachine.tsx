  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white mb-1">Trade Machine</h2>
          <p className="text-sm text-gray-400">Build two-team trades with all NBA teams from `nba_api`, live rosters, salary checks, and asset estimates.</p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
          ...
          {loading ? <div className="py-6 flex justify-center"><BasketballLoader /></div> : (
            <>
              ...
              <div className="rounded-lg bg-gray-800/50 p-3 text-xs text-gray-400 leading-relaxed">
...
function TeamTradePanel({ team, teams, selectedTeamId, onTeamChange, roster, selected, onToggle, picks, selectedPicks, onTogglePick }: {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
      ...
      <div className="mb-4 rounded-lg border border-gray-800 bg-gray-800/30 p-3">
        ...
        <div className="space-y-2">
          {picks.map(pick => {
            ...
            return (
              <button key={pick.id} onClick={() => onTogglePick(pick.id)} className={`w-full flex items-center justify-between gap-3 rounded-lg border px-3 py-2 text-left transition-colors ${checked ? 'bg-orange-500/10 border-orange-500/40' : 'bg-gray-900/50 border-gray-800 hover:border-gray-700'}`}>
                <div>
      ...
      <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
        {roster.map(player => {
          ...
          return (
            <button key={`${team.id}-${playerId}`} onClick={() => onToggle(playerId)} className={`w-full flex items-center gap-3 p-3 rounded-lg border text-left transition-colors ${checked ? 'bg-orange-500/10 border-orange-500/40' : 'bg-gray-800/40 border-gray-800 hover:border-gray-700'}`}>
        ...
        {roster.length === 0 && <div className="text-center py-10 text-xs text-gray-500">No roster data available</div>}
      </div>
    </div>
  );
}
...
function PackageSummary({ title, players, picks, salary, impact }: { title: string; players: Player[]; picks: DraftPick[]; salary: number; impact: number }) {
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
      ...
      <div className="text-xs text-gray-400">{[...players.map(getPlayerName), ...picks.map(pick => pick.label)].join(', ') || 'No assets selected'}</div>
    </div>
  );
}
...
function MiniStat({ label, value }: { label: string; value: string | number }) {
  return <div className="rounded-lg bg-gray-800/50 p-3 text-center"><div className="text-sm font-bold text-white">{value}</div><div className="text-[10px] text-gray-500">{label}</div></div>;
}
