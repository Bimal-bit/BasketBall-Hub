...
export default function Standings() {
  ...
  return (
    <div className="w-full space-y-8 pb-24 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 bg-gray-900 p-4 sm:p-6 rounded-2xl border border-gray-800 backdrop-blur-xl">
        <div>
          <h1 className="text-xl font-bold text-white uppercase tracking-tight">League Standings</h1>
          <p className="text-[10px] text-gray-500 font-medium uppercase tracking-[0.2em] mt-1">NBA Official Season {season}</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
          <div className="flex items-center gap-3 bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5">
            ...
            <select value={season} onChange={e => setSeason(e.target.value)} className="bg-transparent text-white text-sm font-medium uppercase outline-none cursor-pointer w-full">
              {seasons.map(s => <option key={s} value={s} className="bg-gray-900 text-white">{s}</option>)}
            </select>
          </div>
          <div className="flex w-full overflow-x-auto bg-gray-800 border border-gray-700 rounded-xl p-1 sm:w-auto">
            {['Regular Season', 'Playoffs', 'Finals'].map(type => (
              <button key={type} onClick={() => setSeasonType(type)} className={`min-w-max flex-1 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${seasonType === type ? 'bg-orange-500 text-white shadow-none' : 'text-gray-400 hover:text-white'}`}>
                {type.split(' ')[0]}
              </button>
            ))}
          </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {/* Mobile Conference Toggle Selector */}
          <div className="flex md:hidden w-full bg-gray-900 border border-gray-800 rounded-xl p-1">
            <button
              onClick={() => setActiveConf('East')}
              className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all ${
                activeConf === 'East' ? 'bg-orange-500 text-white' : 'text-gray-400'
              }`}
            >
              Eastern Conference
            </button>
            <button
              ...
              className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all ${
                activeConf === 'West' ? 'bg-orange-500 text-white' : 'text-gray-400'
              }`}
            >
              Western Conference
            </button>
...
function ConferenceTable({ title, teams, onTeamClick }: any) {
  return (
    <div className="space-y-3">
      ...
      {/* Table Header */}
      <div className="flex items-center text-[10px] font-bold text-gray-500 uppercase tracking-wider px-3 py-1.5 border-b border-gray-800">
        <div className="w-8 shrink-0">Rank</div>
        <div className="flex-1 min-w-0">Team</div>
        <div className="w-16 text-center shrink-0">W-L</div>
        <div className="w-16 text-right shrink-0">PCT</div>
      </div>

      {/* Table Body - Cards/Rows */}
      <div className="space-y-2">
        {teams.map((team: any, i: number) => (
          <div
            key={team.TeamID}
            onClick={() => onTeamClick(team)}
            className="flex items-center px-3 py-3 rounded-2xl border border-gray-800 bg-gray-900 hover:border-gray-700 transition-colors cursor-pointer w-full"
          >
            {/* Rank */}
            <div className="w-8 shrink-0 text-sm font-bold text-gray-400">
              {i + 1}
            </div>

            {/* Team Logo + Name */}
            <div className="flex-1 flex items-center gap-2.5 min-w-0">
              <img src={getTeamLogoUrl(team.TeamID)} className="w-8 h-8 object-contain shrink-0" alt="" />
            ...
            {/* W-L */}
            <div className="w-16 text-center shrink-0 text-sm font-medium text-gray-300 tabular-nums">
              {team.Wins}-{team.Losses}
            </div>

            {/* PCT */}
            <div className="w-16 text-right shrink-0 text-sm font-bold text-orange-500 tabular-nums">
              {((team.WinPCT || 0) * 100).toFixed(1)}%
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex flex-col items-center gap-4 pt-10 min-w-[140px]">
          ...
          {finalsSeries ? (
            <div className="rounded-2xl border border-orange-500/30 bg-orange-500/10 overflow-hidden shadow-none w-full">
              ...
              <div className="h-px bg-gray-900 bg-gray-900" />
            ...
            <div className="rounded-2xl border border-gray-800 bg-gray-900 p-4 text-center text-xs text-gray-500 font-medium uppercase tracking-widest w-full">
              TBD
            </div>
          )}
        </div>
...
function ConferenceBracket({ title, series, onTeamClick, mirrored = false }: any) {
  ...
  return (
    <section className="rounded-3xl border border-gray-800 bg-gray-900 p-4 sm:p-6">
      <h2 className={`mb-5 text-xl sm:text-2xl font-medium text-gray-500 text-white uppercase tracking-tighter ${mirrored ? 'text-right' : ''}`}>{title}</h2>
...
function RoundColumn({ title, series, onTeamClick, mirrored = false }: any) {
  return (
    <div className="flex min-w-0 flex-col gap-4">
      <h3 className={`text-[10px] font-medium text-gray-500 uppercase tracking-[0.24em] md:tracking-[0.35em] ${mirrored ? 'md:text-right' : 'md:text-center'}`}>{title}</h3>
      <div className="flex flex-col gap-3 justify-center flex-1">
        {series.map((s: any) => (
          <div key={s.seriesId} className="rounded-2xl border border-gray-800 bg-gray-900 bg-gray-950 overflow-hidden shadow-none">
        ...
        {series.length === 0 && (
          <div className="rounded-2xl border border-gray-800 bg-gray-900 bg-gray-950 p-4 text-center text-xs text-gray-500 font-medium uppercase tracking-widest">TBD</div>
        )}
      </div>
    </div>
  );
...
function SeriesRow({ id, name, wins, onClick, isWinner, mirrored = false }: any) {
  return (
    <div onClick={onClick} className={`flex items-center justify-between gap-3 p-3 sm:p-4 cursor-pointer hover:bg-gray-900 dark:hover:bg-gray-900/50 transition-all ${isWinner ? 'bg-orange-500/10' : ''} ${mirrored ? 'flex-row-reverse' : ''}`}>
       <div className={`flex min-w-0 items-center gap-3 ${mirrored ? 'flex-row-reverse' : ''}`}>
          ...
          <span className={`truncate text-xs font-medium uppercase tracking-tight ${isWinner ? 'text-gray-500 text-white' : 'text-gray-500'} ${mirrored ? 'text-right' : ''}`}>{name}</span>
       </div>
       <span className={`shrink-0 text-2xl font-medium ${isWinner ? 'text-orange-500' : 'text-gray-500 '}`}>{wins}</span>
    </div>
  );
}

function FinalsView({ series, onTeamClick, onGameClick }: any) {
  if (!series) return <div className="py-40 text-center text-gray-500  font-medium uppercase tracking-widest">Finals Matchup Pending</div>;
  return (
    <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
      <div className="relative overflow-hidden rounded-3xl sm:rounded-[3rem] border border-orange-500/20 bg-gray-900 p-4 sm:p-8 xl:p-12 text-center shadow-none">
        ...
        <div className="relative z-10 flex flex-col items-center gap-5 sm:gap-8">
          <div className="grid w-full grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-8">
             <div onClick={() => onTeamClick({ TeamID: series.homeId, name: series.homeName })} className="min-w-0 cursor-pointer group text-center">
                ...
                <div className="text-base sm:text-2xl lg:text-3xl font-medium text-gray-500 text-white uppercase tracking-tighter mb-2 break-words">{series.homeName}</div>
                <div className="text-4xl sm:text-7xl font-medium text-orange-500 tabular-nums drop-shadow-none">{series.homeWins}</div>
             </div>
             <div className="text-2xl sm:text-6xl lg:text-7xl font-medium text-gray-500  select-none">VS</div>
             <div onClick={() => onTeamClick({ TeamID: series.visitorId, name: series.visitorName })} className="min-w-0 cursor-pointer group text-center">
                ...
                <div className="text-base sm:text-2xl lg:text-3xl font-medium text-gray-500 text-white uppercase tracking-tighter mb-2 break-words">{series.visitorName}</div>
                <div className="text-4xl sm:text-7xl font-medium text-orange-500 tabular-nums drop-shadow-none">{series.visitorWins}</div>
             </div>
          </div>
          <div className="text-[10px] sm:text-sm font-medium text-gray-500 uppercase tracking-[0.18em] sm:tracking-[0.55em]">NBA Finals Championship</div>
        </div>
      </div>
        ...
        {series.games.map((g: any, i: number) => {
          ...
          return (
            <button 
              ...
              disabled={!hasScore}
              className={`p-4 sm:p-6 rounded-2xl border border-gray-800 bg-gray-900 bg-gray-950 transition-all text-left group ${hasScore ? 'hover:border-orange-500/50 hover:bg-orange-500/5 cursor-pointer' : 'opacity-40 cursor-default'}`}
            >
              <div className="text-[10px] font-medium text-gray-500 tracking-widest mb-6 uppercase">Game {i+1}</div>
              <div className={`text-3xl font-medium ${hasScore ? 'text-gray-500 text-white group-hover:text-orange-400' : 'text-gray-500 '}`}>
                 {hasScore ? `${hScore} - ${vScore}` : 'TBD'}
              </div>
              <div className="text-xs font-medium text-gray-500 uppercase tracking-[0.2em] mt-4">{g.GAME_DATE || 'SCHEDULED'}</div>
            </button>
          );
        })}
      </div>
  ...
  return (
    <div id="team-log-modal" className="fixed inset-0 z-[100] overflow-y-auto bg-black/60 backdrop-blur-md p-2 sm:p-4">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-6xl rounded-3xl sm:rounded-[4rem] border border-gray-800 bg-gray-900 flex flex-col shadow-none">
          <div className="p-4 sm:p-8 border-b border-gray-800 flex items-center justify-between gap-4 bg-gray-900 dark:bg-gray-900/40">
            <div className="flex min-w-0 items-center gap-3 sm:gap-6">
              <img src={getTeamLogoUrl(team.TeamID)} className="w-12 h-12 sm:w-20 sm:h-20 object-contain shrink-0" alt="" />
              <h2 className="text-xl sm:text-4xl font-medium text-gray-500 text-white uppercase tracking-tighter truncate">{team.TeamCity} {team.TeamName}</h2>
            </div>
            <button onClick={onClose} className="w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-gray-900 bg-gray-900 text-gray-500 hover:text-gray-500 hover:text-white transition-all shrink-0 flex items-center justify-center border border-gray-800"><X size={20} /></button>
          </div>
          <div className="flex-1 p-3 sm:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {loading ? (
              ...
              games.map((g: any) => (
                <button key={g.GAME_ID} onClick={() => onGameClick(g)} className="flex items-center justify-between gap-3 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-gray-800 bg-gray-900 bg-gray-900/30 hover:border-orange-500/50 transition-transform duration-200 hover:scale-105 shadow-none hover:shadow-none text-left">
                  <div className="min-w-0">
                    <div className="text-[10px] font-medium text-orange-500 uppercase tracking-widest">{g.GAME_DATE}</div>
                    <div className="text-base font-medium text-gray-500 text-white uppercase truncate">{g.MATCHUP}</div>
                  </div>
...
function BoxScoreModal({ game, boxScore, loading, onPlayerClick, onClose }: any) {
  ...
  return (
    <div id="box-score-modal" className="fixed inset-0 z-[110] overflow-y-auto bg-black/60 backdrop-blur-md p-2 sm:p-4 animate-in slide-in- duration-500">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-7xl rounded-3xl sm:rounded-[4rem] border border-gray-800 bg-gray-900 flex flex-col shadow-none">
          <div className="p-4 sm:p-10 bg-gray-950 border-b border-gray-800 flex flex-col items-center">
            <button onClick={onClose} className="absolute top-4 right-4 sm:top-8 sm:right-8 text-gray-500 hover:text-gray-500 hover:text-white bg-gray-900 bg-gray-900 p-2.5 rounded-full z-20 border border-gray-800"><X size={20} /></button>
            <div className="pr-10 text-3xl sm:text-6xl font-medium text-gray-500 text-white uppercase tracking-tighter mb-4 sm:mb-8 text-center truncate max-w-full">{game.MATCHUP}</div>
            <div className="flex max-w-full gap-2 overflow-x-auto p-2 bg-gray-900 bg-gray-900 rounded-2xl sm:rounded-[2.5rem] border border-gray-800 shadow-none">
              {teamIds.map(tid => (
                <button key={tid} onClick={() => setTeamFilter(tid)} className={`flex shrink-0 items-center gap-3 px-4 sm:px-8 py-3 sm:py-5 rounded-xl sm:rounded-[2rem] transition-all ${teamFilter === tid ? 'bg-orange-500 text-white shadow-none' : 'text-gray-500 hover:text-gray-500  hover:text-white'}`}>
          ...
          <div className="flex-1 p-3 sm:p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
            {loading ? (
              ...
              filtered.map((p: any) => (
                <button key={p.PLAYER_ID} onClick={() => onPlayerClick(p)} className="flex items-center gap-3 sm:gap-5 p-3 sm:p-5 rounded-2xl sm:rounded-[2rem] border border-gray-800 bg-gray-900 bg-gray-900/30 hover:border-orange-500/40 transition-transform duration-200 hover:scale-105 shadow-none hover:shadow-none text-left group">
                  <img src={getPlayerHeadshotUrl(p.PLAYER_ID)} className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-[2rem] object-cover bg-gray-900 bg-gray-900 shadow-none group-hover:scale-110 transition-transform" alt="" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm sm:text-base font-medium text-gray-500 text-white uppercase truncate group-hover:text-orange-400">{p.PLAYER_NAME}</div>
                    <div className="text-[10px] text-gray-500 font-medium uppercase mt-1 tracking-widest">{p.START_POSITION || 'BENCH'}</div>
                  </div>
                  <div className="text-right border-0">
                    <div className="text-2xl sm:text-4xl font-medium text-orange-500 tabular-nums">{p.PTS || 0}</div>
                    <div className="text-[10px] text-gray-500  font-medium uppercase">PTS</div>
                  </div>
                </button>
              ))
            }
    ...
    <div id="player-stats-modal" className="fixed inset-0 z-[120] overflow-y-auto bg-black/60 backdrop-blur-md p-2 sm:p-4 animate-in zoom-in-95 duration-300">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-3xl rounded-3xl sm:rounded-[4rem] border border-gray-800 bg-gray-900 shadow-none flex flex-col">
          <div className="h-36 sm:h-72 bg-gray-900 bg-gray-900 to-transparent relative p-4 sm:p-12 flex items-end gap-4 sm:gap-10 pr-14 rounded-t-[3rem] sm:rounded-t-[4rem]">
            <button onClick={onClose} className="absolute top-4 right-4 sm:top-8 sm:right-8 text-gray-500 hover:text-gray-500 hover:text-white bg-gray-900 bg-gray-900 p-2.5 rounded-full z-20 border border-gray-800"><X size={20} /></button>
            <img src={getPlayerHeadshotUrl(player.PLAYER_ID)} className="w-24 h-24 sm:w-56 sm:h-56 rounded-2xl sm:rounded-[4rem] object-cover bg-gray-900 bg-gray-900 border-4 sm:border-8 border-gray-800 shadow-none translate-y-8 sm:translate-y-24" alt="" />
            <h2 className="text-2xl sm:text-6xl font-medium text-gray-500 text-white uppercase tracking-tighter pb-2 sm:pb-6 leading-none truncate">{player.PLAYER_NAME}</h2>
          </div>
          <div className="flex-1 p-3 sm:p-12 pt-10 sm:pt-32 space-y-6 sm:space-y-12">
            ...
            <div className="space-y-4 sm:space-y-8">
              <h3 className="text-[10px] sm:text-[11px] font-medium text-gray-500 uppercase tracking-[0.2em] sm:tracking-[0.5em] border-b border-gray-800 pb-4 sm:pb-6">Official Performance Feed</h3>
              <div className="space-y-2 sm:space-y-4">
                {loading ? <div className="py-12 flex justify-center"><Loader2 className="w-10 h-10 text-orange-500 animate-spin" /></div> :
                  stats.map((s, i) => (
                    <div key={i} className="flex items-center justify-between gap-3 p-3 sm:p-6 rounded-2xl sm:rounded-[2.5rem] bg-gray-900 bg-gray-900/30 border border-gray-800 hover:bg-gray-900 dark:hover:bg-gray-900 transition-all">
                      <div className="text-[9px] sm:text-[10px] text-gray-500 w-20 sm:w-28 uppercase font-medium tracking-widest shrink-0">{s.GAME_DATE}</div>
                      <div className="text-xs font-medium text-gray-500 text-white flex-1 uppercase tracking-tight truncate">{s.MATCHUP}</div>
...
function StatPill({ val, label }: any) {
  return (
    <div className="p-2 sm:p-6 rounded-xl sm:rounded-[2.5rem] bg-gray-900 bg-gray-950 border border-gray-800 text-center shadow-none min-w-0">
      <div className="text-xl sm:text-4xl font-medium text-gray-500 text-white drop-shadow-none tabular-nums">{val || 0}</div>
      <div className="text-[8px] sm:text-[10px] text-gray-500 font-medium uppercase mt-1 sm:mt-3 tracking-widest truncate">{label}</div>
    </div>
  );
}
