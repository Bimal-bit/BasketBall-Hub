...
function ConferenceTable({ title, teams, onTeamClick }: any) {
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto w-full rounded-2xl border border-white/5 bg-slate-900/20 backdrop-blur-md">
        <table className="min-w-[560px] w-full text-left text-xs sm:text-sm">
          <thead>
            <tr className="bg-white/5 border-b border-white/5">
              <th className="sticky left-0 z-10 bg-[var(--surface)] px-2 py-2 text-[10px] font-black uppercase text-gray-500 sm:px-4">Team</th>
          ...
          <tbody className="divide-y divide-white/[0.02]">
            {teams.map((team: any, i: number) => (
              <tr key={team.TeamID} onClick={() => onTeamClick(team)} className="group hover:bg-white/5 transition-all cursor-pointer">
                <td className="sticky left-0 z-10 bg-[var(--surface)] px-2 py-2 sm:px-4">
...
function TeamLogModal({ team, games, loading, onGameClick, onClose }: any) {
  ...
  return (
    <div id="team-log-modal" className="fixed inset-0 z-[100] overflow-y-auto bg-black/98 backdrop-blur-3xl p-2 sm:p-4">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-6xl rounded-3xl sm:rounded-[4rem] border border-white/10 bg-slate-950 flex flex-col shadow-2xl">
          ...
          <div className="flex-1 p-3 sm:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {loading ? (
              ...
              games.map((g: any) => (
                <button key={g.GAME_ID} onClick={() => onGameClick(g)} className="flex items-center justify-between gap-3 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-white/5 bg-white/[0.02] hover:border-orange-500/50 transition-transform duration-200 hover:scale-105 shadow-md hover:shadow-lg text-left">
...
function BoxScoreModal({ game, boxScore, loading, onPlayerClick, onClose }: any) {
  ...
  return (
    <div id="box-score-modal" className="fixed inset-0 z-[110] overflow-y-auto bg-black/99 backdrop-blur-3xl p-2 sm:p-4 animate-in slide-in-from-bottom-20 duration-500">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-7xl rounded-3xl sm:rounded-[4rem] border border-white/10 bg-slate-950 flex flex-col shadow-2xl">
          ...
          <div className="flex-1 p-3 sm:p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
            {loading ? (
              ...
              filtered.map((p: any) => (
                <button key={p.PLAYER_ID} onClick={() => onPlayerClick(p)} className="flex items-center gap-3 sm:gap-5 p-3 sm:p-5 rounded-2xl sm:rounded-[2rem] border border-white/5 bg-white/[0.03] hover:border-orange-500/40 transition-transform duration-200 hover:scale-105 shadow-md hover:shadow-lg text-left group">
...
function PlayerStatsModal({ player, season, onClose }: any) {
  ...
  return (
    <div id="player-stats-modal" className="fixed inset-0 z-[120] overflow-y-auto bg-black/99 backdrop-blur-3xl p-2 sm:p-4 animate-in zoom-in-95 duration-300">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-3xl rounded-3xl sm:rounded-[4rem] border border-white/10 bg-slate-950 shadow-2xl flex flex-col">
