  ...
  return (
    <div className="w-full space-y-8 pb-20 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 bg-white dark:bg-zinc-900/40 p-4 sm:p-8 rounded-3xl sm:rounded-[3rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] backdrop-blur-xl">
        <div>
          <h1 className="text-2xl sm:text-4xl font-medium text-white  uppercase tracking-tighter">League Standings</h1>
          <p className="text-[9px] sm:text-[10px] text-gray-500 font-medium uppercase tracking-[0.2em] sm:tracking-[0.5em] mt-2">NBA Official Season {season}</p>
        </div>
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center">
          <div className="flex items-center gap-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl px-4 py-3">
          ...
          <div className="flex w-full overflow-x-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 border-[0.5px] rounded-2xl p-1.5 sm:w-auto">
            {['Regular Season', 'Playoffs', 'Finals'].map(type => (
              <button key={type} onClick={() => setSeasonType(type)} className={`min-w-max flex-1 px-4 sm:px-8 py-3 rounded-xl text-[10px] font-medium uppercase tracking-widest transition-all ${seasonType === type ? 'bg-orange-500 text-white shadow-none' : 'text-gray-500 hover:text-white'}`}>
                {type.split(' ')[0]}
              </button>
            ))}
          </div>
...
function ConferenceTable({ title, teams, onTeamClick }: any) {
  return (
    <div className="space-y-6">
      <h2 className="px-2 text-lg font-medium text-white sm:text-xl">{title}</h2>
      <div className="overflow-x-auto w-full rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900/20 backdrop-">
        <table className="min-w-[560px] w-full text-left text-xs sm:text-sm">
          <thead>
            <tr className="bg-white/5 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
              <th className="sticky left-0 z-10 bg-white dark:bg-zinc-900 px-2 py-1 text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Team</th>
              <th className="px-2 py-1 text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">Rank</th>
              <th className="px-2 py-1 text-center text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">W-L</th>
              <th className="px-2 py-1 text-center text-[10px] uppercase tracking-[0.5px] text-zinc-400 font-medium border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">PCT</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/[0.02]">
            {teams.map((team: any, i: number) => (
              <tr key={team.TeamID} onClick={() => onTeamClick(team)} className="group hover:bg-white/5 transition-all cursor-pointer">
                <td className="sticky left-0 z-10 bg-white dark:bg-zinc-900 px-2 py-2 text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">
                  <div className="flex items-center gap-3 sm:gap-6">
                    ...
                    <div className="max-w-[120px] truncate text-sm font-medium uppercase text-white group-hover:text-orange-400 sm:max-w-none sm:text-base">{team.TeamCity} {team.TeamName}</div>
                  </div>
                </td>
                <td className="px-2 py-2 text-xs text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px]">{i + 1}</td>
                <td className="whitespace-nowrap px-2 py-2 text-center font-medium  text-white tabular-nums sm:px-4">{team.Wins}-{team.Losses}</td>
                <td className="px-2 py-2 text-center font-medium  text-orange-400 tabular-nums sm:px-4">{((team.WinPCT || 0) * 100).toFixed(1)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="space-y-8">
      ...
      <div className="hidden xl:grid xl:grid-cols-[1fr_auto_1fr] gap-4 items-start">
        ...
        <div className="flex flex-col items-center gap-4 pt-10 min-w-[140px]">
          ...
          {finalsSeries ? (
            <div className="rounded-2xl border border-orange-500/30 bg-orange-500/5 overflow-hidden shadow-none w-full">
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900/40 p-4 text-center text-xs text-gray-600 font-medium uppercase tracking-widest w-full">
              TBD
            </div>
          )}
        </div>
...
function ConferenceBracket({ title, series, onTeamClick, mirrored = false }: any) {
  ...
  return (
    <section className="rounded-3xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900/20 p-4 sm:p-6">
      <h2 className={`mb-5 text-xl sm:text-2xl font-medium text-white  uppercase tracking-tighter ${mirrored ? 'text-right' : ''}`}>{title}</h2>
...
function RoundColumn({ title, series, onTeamClick, mirrored = false }: any) {
  return (
    <div className="flex min-w-0 flex-col gap-4">
      <h3 className={`text-[10px] font-medium text-gray-600 uppercase tracking-[0.24em] md:tracking-[0.35em] ${mirrored ? 'md:text-right' : 'md:text-center'}`}>{title}</h3>
      <div className="flex flex-col gap-3 justify-center flex-1">
        {series.map((s: any) => (
          <div key={s.seriesId} className="rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900/40 overflow-hidden shadow-none backdrop-">
        ...
        {series.length === 0 && (
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900/20 p-4 text-center text-xs text-gray-700 font-medium uppercase tracking-widest">TBD</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClick} className={`flex items-center justify-between gap-3 p-3 sm:p-4 cursor-pointer hover:bg-white/5 transition-all ${isWinner ? 'bg-orange-500/10' : ''} ${mirrored ? 'flex-row-reverse' : ''}`}>
       <div className={`flex min-w-0 items-center gap-3 ${mirrored ? 'flex-row-reverse' : ''}`}>
          ...
          <span className={`truncate text-xs font-medium uppercase tracking-tight ${isWinner ? 'text-white' : 'text-gray-500'} ${mirrored ? 'text-right' : ''}`}>{name}</span>
       </div>
       <span className={`shrink-0 text-2xl font-medium  ${isWinner ? 'text-orange-500' : 'text-gray-700'}`}>{wins}</span>
    </div>
  );
}

function FinalsView({ series, onTeamClick, onGameClick }: any) {
  if (!series) return <div className="py-40 text-center text-gray-700 font-medium uppercase tracking-widest">Finals Matchup Pending</div>;
  return (
        ...
        <div className="relative z-10 flex flex-col items-center gap-5 sm:gap-8">
          <div className="grid w-full grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-8">
             <div onClick={() => onTeamClick({ TeamID: series.homeId, name: series.homeName })} className="min-w-0 cursor-pointer group text-center">
                ...
                <div className="text-base sm:text-2xl lg:text-3xl font-medium text-white uppercase  tracking-tighter mb-2 break-words">{series.homeName}</div>
                <div className="text-4xl sm:text-7xl font-medium text-orange-500  tabular-nums drop-shadow-none">{series.homeWins}</div>
             </div>
             <div className="text-2xl sm:text-6xl lg:text-7xl font-medium text-gray-900  select-none">VS</div>
             <div onClick={() => onTeamClick({ TeamID: series.visitorId, name: series.visitorName })} className="min-w-0 cursor-pointer group text-center">
                ...
                <div className="text-base sm:text-2xl lg:text-3xl font-medium text-white uppercase  tracking-tighter mb-2 break-words">{series.visitorName}</div>
        ...
        {series.games.map((g: any, i: number) => {
          ...
          return (
            <button 
              ...
              disabled={!hasScore}
              className={`p-4 sm:p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white/5 transition-all text-left group ${hasScore ? 'hover:border-orange-500/50 hover:bg-orange-500/5 cursor-pointer' : 'opacity-40 cursor-default'}`}
            >
              <div className="text-[10px] font-medium text-gray-600 uppercase tracking-widest mb-6">Game {i+1}</div>
              <div className={`text-3xl font-medium  ${hasScore ? 'text-white group-hover:text-orange-400' : 'text-gray-500'}`}>
                 {hasScore ? `${hScore} - ${vScore}` : 'TBD'}
              </div>
              <div className="text-xs font-medium text-gray-700 uppercase tracking-[0.2em] mt-4">{g.GAME_DATE || 'SCHEDULED'}</div>
            </button>
          );
        })}
      </div>
...
function TeamLogModal({ team, games, loading, onGameClick, onClose }: any) {
  ...
  return (
    <div id="team-log-modal" className="fixed inset-0 z-[100] overflow-y-auto bg-white dark:bg-zinc-900 backdrop-blur-3xl p-2 sm:p-4">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-6xl rounded-3xl sm:rounded-[4rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900 flex flex-col shadow-none">
          <div className="p-4 sm:p-8 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px] flex items-center justify-between gap-4 bg-white dark:bg-zinc-900/40">
            <div className="flex min-w-0 items-center gap-3 sm:gap-6">
              <img src={getTeamLogoUrl(team.TeamID)} className="w-12 h-12 sm:w-20 sm:h-20 object-contain shrink-0" alt="" />
              <h2 className="text-xl sm:text-4xl font-medium text-white  uppercase tracking-tighter truncate">{team.TeamCity} {team.TeamName}</h2>
            </div>
          ...
          <div className="flex-1 p-3 sm:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {loading ? (
              ...
              games.map((g: any) => (
                <button key={g.GAME_ID} onClick={() => onGameClick(g)} className="flex items-center justify-between gap-3 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white/[0.02] hover:border-orange-500/50 transition-transform duration-200 hover:scale-105 shadow-none hover:shadow-none text-left">
                  <div className="min-w-0">
                    <div className="text-[10px] font-medium text-orange-500 uppercase tracking-widest">{g.GAME_DATE}</div>
                    <div className="text-base font-medium text-white uppercase truncate">{g.MATCHUP}</div>
                  </div>
...
function BoxScoreModal({ game, boxScore, loading, onPlayerClick, onClose }: any) {
  ...
  return (
    <div id="box-score-modal" className="fixed inset-0 z-[110] overflow-y-auto bg-white dark:bg-zinc-900 backdrop-blur-3xl p-2 sm:p-4 animate-in slide-in- duration-500">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-7xl rounded-3xl sm:rounded-[4rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900 flex flex-col shadow-none">
          <div className="p-4 sm:p-10 bg-white dark:bg-zinc-900/60 border-b border-zinc-200 dark:border-zinc-800 border-[0.5px] flex flex-col items-center">
            ...
            <div className="pr-10 text-3xl sm:text-6xl font-medium text-white  uppercase tracking-tighter mb-4 sm:mb-8 text-center truncate max-w-full">{game.MATCHUP}</div>
            <div className="flex max-w-full gap-2 overflow-x-auto p-2 bg-white dark:bg-zinc-900 rounded-2xl sm:rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] shadow-none">
              {teamIds.map(tid => (
                <button key={tid} onClick={() => setTeamFilter(tid)} className={`flex shrink-0 items-center gap-3 px-4 sm:px-8 py-3 sm:py-5 rounded-xl sm:rounded-[2rem] transition-all ${teamFilter === tid ? 'bg-orange-500 text-white shadow-none' : 'text-gray-500 hover:text-white'}`}>
          ...
          <div className="flex-1 p-3 sm:p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
            {loading ? (
              ...
              filtered.map((p: any) => (
                <button key={p.PLAYER_ID} onClick={() => onPlayerClick(p)} className="flex items-center gap-3 sm:gap-5 p-3 sm:p-5 rounded-2xl sm:rounded-[2rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white/[0.03] hover:border-orange-500/40 transition-transform duration-200 hover:scale-105 shadow-none hover:shadow-none text-left group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm sm:text-base font-medium text-white uppercase truncate group-hover:text-orange-400">{p.PLAYER_NAME}</div>
                    <div className="text-[10px] text-gray-600 font-medium uppercase mt-1 tracking-widest">{p.START_POSITION || 'BENCH'}</div>
                  </div>
                  <div className="text-right border-0">
                    <div className="text-2xl sm:text-4xl font-medium text-orange-500 tabular-nums">{p.PTS || 0}</div>
                    <div className="text-[10px] text-gray-700 font-medium uppercase">PTS</div>
                  </div>
                </button>
              ))
            }
...
function PlayerStatsModal({ player, season, onClose }: any) {
  ...
  return (
    <div id="player-stats-modal" className="fixed inset-0 z-[120] overflow-y-auto bg-white dark:bg-zinc-900 backdrop-blur-3xl p-2 sm:p-4 animate-in zoom-in-95 duration-300">
      <div className="flex min-h-full items-end sm:items-center justify-center">
        <div className="relative w-full max-w-3xl rounded-3xl sm:rounded-[4rem] border border-zinc-200 dark:border-zinc-800 border-[0.5px] bg-white dark:bg-zinc-900 shadow-none flex flex-col">
          <div className="h-36 sm:h-72 bg-zinc-100 dark:bg-zinc-800  to-transparent relative p-4 sm:p-12 flex items-end gap-4 sm:gap-10 pr-14">
            ...
            <h2 className="text-2xl sm:text-6xl font-medium text-white  uppercase tracking-tighter pb-2 sm:pb-6 leading-none truncate">{player.PLAYER_NAME}</h2>
          </div>
          <div className="flex-1 p-3 sm:p-12 pt-10 sm:pt-32 space-y-6 sm:space-y-12">
            ...
            <div className="space-y-4 sm:space-y-8">
              <h3 className="text-[10px] sm:text-[11px] font-medium text-gray-700 uppercase tracking-[0.2em] sm:tracking-[0.5em] border-b border-zinc-200 dark:border-zinc-800 border-[0.5px] pb-4 sm:pb-6">Official Performance Feed</h3>
              <div className="space-y-2 sm:space-y-4">
                {loading ? <div className="py-12 flex justify-center"><Loader2 className="w-10 h-10 text-orange-500 animate-spin" /></div> :
                  stats.map((s, i) => (
                    <div key={i} className="flex items-center justify-between gap-3 p-3 sm:p-6 rounded-2xl sm:rounded-[2.5rem] bg-white/[0.03] border border-zinc-200 dark:border-zinc-800 border-[0.5px] hover:bg-white/[0.05] transition-all">
                      ...
                      <div className="text-xs font-medium text-white flex-1 uppercase tracking-tight truncate">{s.MATCHUP}</div>
...
function StatPill({ val, label }: any) {
  return (
    <div className="p-2 sm:p-6 rounded-xl sm:rounded-[2.5rem] bg-white/5 border border-zinc-200 dark:border-zinc-800 border-[0.5px] text-center shadow-none min-w-0">
      ...
      <div className="text-[8px] sm:text-[10px] text-gray-700 font-medium uppercase mt-1 sm:mt-3 tracking-widest truncate">{label}</div>
    </div>
  );
}
